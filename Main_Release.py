    ...
    while True:
        ...
        for enm_card in Ingame_enm_cards:
            ...
            if in_range_tower:
                ...
                enm_card.move(destination)
        for tower in Tower_List:
            in_range_enmy = in_range(tower, Ingame_enm_cards)
            closest_distance = (640 ** 2 + 360 ** 2) ** 0.5
            print(len(in_range_enmy))
            closest_enemy = None
            for enemy in in_range_enmy:
                distance = (((enemy.position[0] - 20) - (tower.position[0] - tower.size[0]/2)) ** 2 + (
                        (enemy.position[1] - 23.5) - (tower.position[1] -tower.size[1]/2)) ** 2) ** 0.5
                if distance < closest_distance:
                    closest_enemy = enemy
                    closest_distance = distance
            if not closest_enemy == None:
        ...
        for tower in Other_Towers:
            in_range_enmy = in_range(tower,InGameCards, Tower_List)
            closest_distance = (640 ** 2 + 360 ** 2) ** 0.5
            closest_enemy = None
            for enemy in in_range_enmy:
                distance = (((enemy.position[0] - 20) - (tower.position[0] - tower.size[0] / 2)) ** 2 + (
